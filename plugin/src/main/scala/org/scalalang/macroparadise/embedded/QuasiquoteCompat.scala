...
trait QuasiquoteCompatV2 {
  ...
  // ==================== NEW APIS INTRODUCED IN BUILDUTILS/UNIVERSE ====================
  ...
  object Applied {
    def unapply(tree: Tree): Some[(Tree, List[List[Tree]])] = {
      val TreeInfoApplied(fun, targs, argss) = tree
      targs match {
    ...
    def unapply(tree: Tree): Option[(Modifiers, TypeName, List[TypeDef],
      ...
      case ClassDef(mods, name, tparams, Template(parents, selfdef, tbody)) =>
        ...
        val (argss, superPos) = ctor.rhs match {
          ...
          case Block(_ :+ (superCall @ TreeInfoApplied(core, _, argss)), Literal(Constant(()))) => (argss, core.pos)
  ...
  class TreeInfoApplied(val tree: Tree) {
    def callee: Tree = {
  ...
  object TreeInfoApplied {
    def apply(tree: Tree): TreeInfoApplied = new TreeInfoApplied(tree)

    def unapply(applied: TreeInfoApplied): Option[(Tree, List[Tree], List[List[Tree]])] =
      Some((applied.core, applied.targs, applied.argss))

    def unapply(tree: Tree): Option[(Tree, List[Tree], List[List[Tree]])] =
      unapply(new TreeInfoApplied(tree))
  }
