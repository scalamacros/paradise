...
trait Implicits extends NscImplicits {
  ...
  override def allViewsFrom(tp: Type, context: Context, tpars: List[Symbol]): List[(SearchResult, List[TypeConstraint])] = {
    ...
    val search = new ParadiseImplicitSearch(EmptyTree, functionType(List(tpSubsted), AnyClass.tpe), true, context.makeImplicit(false), NoPosition)
...
//    assert(tree.isEmpty || tree.pos.isDefined, tree)
    override def pos = if (pos0 != NoPosition) pos0 else tree.pos

    override def failure(what: Any, reason: String, pos: Position = this.pos): SearchResult = {
     ...
     */
    override def improves(info1: ImplicitInfo, info2: ImplicitInfo) = {
    ...
    override def isPlausiblyCompatible(tp: Type, pt: Type) = checkCompatibility(fast = true, tp, pt)
    override def normSubType(tp: Type, pt: Type) = checkCompatibility(fast = false, tp, pt)
    ...
    // NOTE: it's a really bad idea to do `override val` here!
    // /** The type parameters to instantiate */
    // override val undetParams = if (isView) List() else context.outer.undetparams
    // /** The expected type with all undetermined type parameters replaced with wildcards. */
    // override def approximate(tp: Type) = deriveTypeWithWildcards(undetParams)(tp)
    // override val wildPt = approximate(pt)
     ...
     *  conditions is true.
     */
    override def isValid(sym: Symbol) = {
     ...
     */
    override def applicableInfos(iss: Infoss, isLocal: Boolean): Map[ImplicitInfo, SearchResult] = {
     ...
     *                        symbols of the same name in succeeding lists.
     */
    override def searchImplicit(implicitInfoss: Infoss, isLocal: Boolean): SearchResult =
    ...
    override def wrapResult(tree: Tree): SearchResult =
     ...
     */
    override def bestImplicit: SearchResult = {
    ...
    override def allImplicits: List[SearchResult] = {
    ...
    override def allImplicitsPoly(tvars: List[TypeVar]): List[(SearchResult, List[TypeConstraint])] = {
