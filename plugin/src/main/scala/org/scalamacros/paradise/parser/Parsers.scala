...
import scala.tools.nsc.ast.parser.{ BracePatch, BracePair, Change, Patch }
...
trait ParsersCommon extends ScannersCommon { self =>
  ...
  import global.{currentUnit => _, gen => _, _}
...
self =>
  val global: Global
  import global.{gen => _, treeInfo => _, build => _, _}
  import compat.{gen, treeInfo, build}
  import compat.symbolTable.{freshTermName, freshTypeName, RichPosition}
  import compat.symbolTable.{RichTree, RichModifiers}
  ...
  class SourceFileParser(val source: SourceFile) extends Parser {
    ...
    private[this] lazy val xmlp = {
      // NOTE: this is unnecessary in 2.10
  ...
  abstract class Parser extends ParserCommon { parser =>
    ...
    import treeBuilder.{global => _, unit => _, source => _, fresh => _, compat => _, _}

    implicit def fresh: FreshNameCreator = scala.quasiquotes.globalFreshNameCreator
...
/* ---------- TREE CONSTRUCTION ------------------------------------------- */
    ...
    def convertToParam(tree: Tree): ValDef = atPos(tree.pos) {
      ...
      def errorParam = makeParam(nme.ERROR, errorTypeTree setPos o2p(tree.pos.endOrPoint))
      tree match {
        case Ident(name) =>
        ...
        case build.SyntacticTuple(as) =>
          ...
          def exampleTuplePattern(names: List[Name]): String = {
          ...
          val example = exampleTuplePattern(as map { case Ident(name) => name; case _ => nme.EMPTY })
          val msg =
...
/* ----------- EXPRESSIONS ------------------------------------------------ */
    ...
    def expr0(location: Location): Tree = (in.token: @scala.annotation.switch) match {
      ...
      case FOR =>
        ...
        def adjustStart(tree: Tree) =
          if (tree.pos.isRange && start < tree.pos.startOrPoint)
    ...
    def makeFilter(start: Offset, tree: Tree) = gen.Filter(tree).setPos(r2p(start, tree.pos.point, tree.pos.endOrPoint))
      ...
      def pattern3(): Tree = {
        ...
        def checkWildStar: Tree = top match {
          case Ident(nme.WILDCARD) if isSequenceOK && isRawStar => peekingAhead (
            if (isCloseDelim) atPos(top.pos.startOrPoint, in.prev.offset)(Star(stripParens(top)))
            else EmptyTree
          )
          case _ => EmptyTree
        }
...
/* -------- PARAMETERS ------------------------------------------- */
    ...
    def typeBounds(): TypeBoundsTree = {
      val lo      = bound(SUPERTYPE, tpnme.Nothing)
      val hi      = bound(SUBTYPE, tpnme.Any)
      ...
      atPos(start, if (name == tpnme.ERROR) start else nameOffset) {
        savingClassContextBounds {
          ...
          if (!classContextBounds.isEmpty && mods.isTrait) {
            syntaxError(s"traits cannot have type parameters with context bounds `: ...' nor view bounds `<% ...'", skipIt = false)
