...
abstract class ReificationSupport extends SymbolTableCompat { self =>
  ...
  protected object MaybeUnchecked {
    def unapply(tree: Tree): Some[Tree] = tree match {
      case Annotated(SyntacticNew(Nil, Apply(ScalaDot(compattpnme.unchecked), Nil) :: Nil, emptyValDef, Nil), annottee) =>
        Some(annottee)
      case Typed(annottee, MaybeTypeTreeOriginal(
        Annotated(SyntacticNew(Nil, Apply(ScalaDot(compattpnme.unchecked), Nil) :: Nil, emptyValDef, Nil), _))) =>
        Some(annottee)
