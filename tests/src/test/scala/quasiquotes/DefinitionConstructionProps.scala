...
trait ClassConstruction { self: QuasiquoteProperties =>
  ...
  def classWith(name: TypeName, parents: List[Tree] = List(anyRef), body: List[DefDef] = Nil) =
    ClassDef(
      Modifiers(), name, List(),
      Template(parents, emptyValDef, List(emtpyConstructor) ++ body))

  property("construct case class") = test {
    val params = q"val x: Int" :: q"val y: Int" :: Nil
    val name = newTypeName("Point")
    assertEqAst(q"$CASE class $name(..$params)", q"case class Point(x: Int, y: Int)")
  }

  property("case class bare param") = test {
    assertEqAst(q"$CASE class Point(x: Int, y: Int)", q"case class Point(private[this] val x: Int, private[this] val y: Int)")
  }

  property("generate default constructors automatically") = test {
    val parents = List.empty[Tree]
    assertEqAst(q"class Foo extends ..$parents", q"class Foo")
  }

  // can't implement without toolbox
  ...
  property("param flags are consistent with raw code") = test {
    ...
    assertEqAst(q"     class C(x: Int)", q"     class C(x: Int)                  ")
    assertEqAst(q"case class C(x: Int)", q"case class C(x: Int)                  ")
    assertEqAst(q"     class C($pubx) ", q"     class C(val x: Int)              ")
    assertEqAst(q"case class C($pubx) ", q"case class C(x: Int)                  ")
    assertEqAst(q"     class C($privx)", q"     class C(x: Int)                  ")
    assertEqAst(q"case class C($privx)", q"case class C(private[this] val x: Int)")
  }
}

trait TraitConstruction { self: QuasiquoteProperties =>
  property("splice name into trait def") = test {
    val Foo = newTypeName("Foo")
  ...
  property("splice early valdef into trait") = test {
    val x = q"val x: Int = 1"
    assertEqAst(q"trait T extends { $x } with Any", q"trait T extends { val x: Int = 1} with Any")
  }

  property("construct trait with early valdef") = test {
    assertEqAst(q"trait T extends { val x: Int = 1 } with Any", q"trait T extends { val x: Int = 1 } with Any")
  }
...
trait MethodConstruction { self: QuasiquoteProperties =>
  ...
  property("splice annotation with targs") = test {
    val a = q"new Foo[A, B]"
    assertEqAst(q"@$a def foo", q"@Foo[A,B] def foo")
  }
