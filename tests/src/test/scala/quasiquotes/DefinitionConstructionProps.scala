...
trait ClassConstruction { self: QuasiquoteProperties =>
  val anyRef = ScalaDot(newTypeName("AnyRef"))
  val emtpyConstructor =
    DefDef(Modifiers(), nme.CONSTRUCTOR, List(),
      List(List()), TypeTree(), Block(List(Apply(Select(Super(This(tpnme.EMPTY), tpnme.EMPTY), nme.CONSTRUCTOR), Nil)), Literal(Constant(()))))
  ...
  property("unquote term name into class") = forAll { (rname: TypeName) =>
    ...
    val name = newTypeName("prefix$" + rname)
