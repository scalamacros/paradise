...
object UnliftableProps extends QuasiquoteProperties("unliftable") {
  property("unlift name") = test {
    val termname0 = newTermName("foo")
    val typename0 = newTypeName("foo")
  ...
  property("don't unlift non-tree unquotee (1)") = test {
    ...
    assert(a == newTermName("a"))
    assert(b == newTermName("b"))
  }
  ...
  property("unlift xml comment") = test {
    def Unliftable[T](pf: PartialFunction[Tree, T]): Unliftable[T] = new Unliftable[T] {
